...
class AsyncTask:
    def __init__(self, args):
        ...
        self.codeformer_alpha=args.pop()
        self.codeformer_upscale=args.pop()
        ...
        # Reemplázalas con placeholders para mantener el orden:
        args.pop()  # photomaker_enabled (elimina sin usar)
        args.pop()  # photomaker_images
        args.pop()  # instantid_enabled
        args.pop()  # instantid_image_path
        args.pop()  # instantid_pose_image_path
        args.pop()  # instantid_identitynet_strength_ratio
        args.pop()  # instantid_adapter_strength_ratio
...
#    from modules.pm import generate_photomaker
    ...
    def process_task(all_steps, async_task, callback, controlnet_canny_path, controlnet_cpds_path, controlnet_ctrl_u_path, current_task_id,
        ...
        if 'cn' in goals:
            for cn_flag, cn_path in [
                ...
                (flags.cn_ctrl_u, controlnet_ctrl_u_path)
    ...
    def apply_control_nets(async_task, height, ip_adapter_face_path, ip_adapter_path, width, current_progress):
        ...
        for task in async_task.cn_tasks[flags.cn_cpds]:
            ...
            if async_task.debugging_cn_preprocessor:
                yield_result(async_task, cn_img, current_progress, async_task.black_out_nsfw, do_not_show_finished_images=True)
        for task in async_task.cn_tasks.get(flags.cn_ctrl_u, []): 
            cn_img, cn_stop, cn_weight = task
            cn_img = resize_image(HWC3(cn_img), width=width, height=height)
    ...
    def apply_image_input(async_task, base_model_additional_loras, clip_vision_path, controlnet_canny_path,
                          controlnet_cpds_path, controlnet_ctrl_u_path, goals, inpaint_head_model_path, inpaint_image, inpaint_mask,
        ...
        if async_task.current_tab == 'ip' or \
            ...
            if len(async_task.cn_tasks[flags.cn_ctrl_u]) > 0:
                controlnet_ctrl_u_path = modules.config.downloading_controlnet_union()
        ...
        return base_model_additional_loras, clip_vision_path, controlnet_canny_path, controlnet_cpds_path, controlnet_ctrl_u_path, inpaint_head_model_path, inpaint_image, inpaint_mask, ip_adapter_face_path, ip_adapter_path, ip_negative_path, skip_prompt_processing, use_synthetic_refiner
    ...
    def process_enhance(all_steps, async_task, callback, controlnet_canny_path, controlnet_cpds_path, controlnet_ctrl_u_path,
        ...
        imgs, img_paths, current_progress = process_task(all_steps, async_task, callback, controlnet_canny_path,
                                                         controlnet_cpds_path, controlnet_ctrl_u_path, current_task_id, denoising_strength,
    ...
    def enhance_upscale(all_steps, async_task, base_progress, callback, controlnet_canny_path, controlnet_cpds_path, controlnet_ctrl_u_path,
        ...
        if len(goals_enhance) > 0:
            try:
                current_progress, img, prompt, negative_prompt = process_enhance(
                    ...
                    controlnet_cpds_path, controlnet_ctrl_u_path, current_progress, current_task_id, denoising_strength, False,
    ...
    def handler(async_task: AsyncTask):
        ...
        if async_task.input_image_checkbox:
            base_model_additional_loras, clip_vision_path, controlnet_canny_path, controlnet_cpds_path, controlnet_ctrl_u_path, inpaint_head_model_path, inpaint_image, inpaint_mask, ip_adapter_face_path, ip_adapter_path, ip_negative_path, skip_prompt_processing, use_synthetic_refiner = apply_image_input(
                async_task, base_model_additional_loras, clip_vision_path, controlnet_canny_path, controlnet_cpds_path, controlnet_ctrl_u_path,
        ...
        pipeline.refresh_controlnets([controlnet_canny_path, controlnet_cpds_path, controlnet_ctrl_u_path])
        ...
        for current_task_id, task in enumerate(tasks):
            ...
            try:
                imgs, img_paths, current_progress = process_task(all_steps, async_task, callback, controlnet_canny_path,
                                                                 controlnet_cpds_path, controlnet_ctrl_u_path, current_task_id,
        ...
        for img in images_to_enhance:
            ...
            if should_process_enhance_uov and async_task.enhance_uov_processing_order == flags.enhancement_uov_before:
                ...
                current_task_id, done_steps_inpainting, done_steps_upscaling, img, exception_result = enhance_upscale(
                    all_steps, async_task, base_progress, callback, controlnet_canny_path, controlnet_cpds_path, controlnet_ctrl_u_path,
                    current_task_id, denoising_strength, done_steps_inpainting, done_steps_upscaling, enhance_steps,
            ...
            for enhance_mask_dino_prompt_text, enhance_prompt, enhance_negative_prompt, enhance_mask_model, enhance_mask_cloth_category, enhance_mask_sam_model, enhance_mask_text_threshold, enhance_mask_box_threshold, enhance_mask_sam_max_detections, enhance_inpaint_disable_initial_latent, enhance_inpaint_engine, enhance_inpaint_strength, enhance_inpaint_respective_field, enhance_inpaint_erode_or_dilate, enhance_mask_invert in async_task.enhance_ctrls:
                ...
                try:
                    current_progress, img, enhance_prompt_processed, enhance_negative_prompt_processed = process_enhance(
                        all_steps, async_task, callback, controlnet_canny_path, controlnet_cpds_path, controlnet_ctrl_u_path,
